...
export class RefreshGuard implements CanActivate {
  constructor(private readonly jwtService: JwtService, private readonly blacklisted: BlacklistedService) {}

  async canActivate(context: ExecutionContext): Promise<boolean> {
    const request = context.switchToHttp().getRequest<AppRequest>();

    const rawToken =
      request.cookies[COOKIE_NAME] ?? request.headers.authorization?.split(/\s+/g)[1] ?? request.body.refresh;

    if (!rawToken) throw new UnauthorizedException('AUTH.INVALID_TOKEN');

    try {
      request.refreshToken = this.jwtService.verify<JwtToken>(AES.decrypt(rawToken), {
        secret: AUTH_CONFIG().REFRESH_TOKEN.secret,
      });

      const isBlacklisted = await this.blacklisted.isBlacklisted(...Object.values(request.refreshToken));
