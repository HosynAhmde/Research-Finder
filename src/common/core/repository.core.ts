...
export class Repository<Schema, CreateDto, UpdateDto> {
  ...
  count(filter: CountFilter<Document & Schema>): Promise<number> {
    return this.model
      .countDocuments(
        sanitizeQuery({ ...filter.query, deletedAt: { $exists: false } }),
      )
      .exec();
  }

  create(createDto: CreateDto): Promise<Document & Schema> {
    return this.model.create({ ...createDto, createdAt: new Date() });
  }
  findOne(
    filter: OneFilter<Document & Schema>,
  ): Promise<Document & Schema> | null {
    return this.model
      .findOne(sanitizeQuery(filter.query), filter.projection)
      .exec();
  }
  find(
  ...
  findById(
    ...
    return this.model
      .findById(sanitizeQuery(filter.query.id), filter.projection)
      .exec();
  }
  ...
  destroyById(
    ...
    return this.model
      .findByIdAndDelete(filter.query.id, { projection: filter.projection })
      .exec();
  }
}
